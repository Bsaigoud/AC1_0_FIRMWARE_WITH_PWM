...
void RFID ::rfid_scan()
{
  ...
  { // new tag is available
    ...
    { // NUID has been readed
      ...
      rfid_read_flag = 1;
#if AC1_0_EVRE
      mcp.digitalWrite(BUZZER_PIN, HIGH); // Turn ON pin 4 of MCP23008
#endif
      ...
      mcp.digitalWrite(BUZZER_PIN, LOW); // Turn off pin 4 of MCP23008
...
#endif
          if ((evse_ChargePointStatus != Faulted) && (EVSE_states == STATE_B))
          {
            EvseDevStatus_connector_1 = falg_EVSE_Authentication;
            gu8_evse_change_state = EVSE_AUTHENTICATION;
